...
get_chip_num()
{
    ...
    chip_num1=${ant_tmp#*chain_acs1=}
    chip_num1=${chip_num1%%,chain_acs2=*}
    chip_num2=${ant_tmp#*chain_acs2=}                                                     
    chip_num2=${chip_num2%%,chain_acs3=*}
    let chip_num=`echo $chip_num1 | grep -o "o" |wc -w`+`echo $chip_num2 | grep -o "o" |wc -w`
}

get_freq()
{
    ant_tmp=`cgminer-api -o stats`   
    echo $ant_tmp > /etc/first_freq    
    freq=${ant_tmp#*frequency=}                                                         
    freq=${freq%%,voltage=*}
    echo $freq >> /etc/first_freq    
}
